...
describe('bundler', () => {
  ...
  describe('bundleScripts', () => {
    ...
    it('compact mode keeps a dynamic island at its emitted URL until activation', async () => {
      const shared = await writeAsset(
      ...
      const page = await writeAsset(
        'assets/page.js',
        `import { store } from './shared-state.js'; globalThis.__pageStore = store; globalThis.__loadCompactIsland = () => import('./island.js');`,
      );
      ...
      manifest['assets/page.js'].dynamicImports = ['assets/island.js'];
      manifest['assets/island.js'].imports = ['assets/shared-state.js'];
      manifest['assets/island.js'].isDynamicEntry = true;
      ...
      const result = await runBundleScripts([page], distClientDir, policy, { compact: true });
      const written = await executeBundle(result.filename);

      expect([...written]).toHaveLength(1);
      expect(result.content).toContain('/assets/island.js');
      expect(globalThis.__islandRuns).toBe(0);
      const loaded = await globalThis.__loadCompactIsland();
      expect(globalThis.__islandRuns).toBe(1);
      expect(loaded.default).toBe(globalThis.__pageStore);
      ...
      delete globalThis.__loadCompactIsland;
    }, 30_000);
  });
